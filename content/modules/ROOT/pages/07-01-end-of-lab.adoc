= Thank you for your participation
include::_attributes.adoc[]

We hope that the materials we used during this time together were useful and gave you a better appreciation and understanding of what goes into an intelligent application.

If you notice https://github.com/ritzshah/insurance-claim-processing-partners/issues[issues,window=_blank] with the content and/or want to send us a https://github.com/ritzshah/insurance-claim-processing-partners/pulls[pull request,window=_blank], we'll appreciate it very much.

== Acknowledgements

Here is a list of people who have participated, directly or indirectly, to the creation and delivery of this lab.

* Guillaume Moutier
* Robert Lundberg
* Audrey Guidera
* Chris Chase
* Prasanth Anbalagan
* Erwan Granger
* Ritesh Shah
