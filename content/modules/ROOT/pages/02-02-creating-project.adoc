...
It is highly recommended to create your pipeline server before creating a workbench. So let's do that now! Note that you may see `Configure Pipeline`` instead of `Create Pipeline``, Just select that and key in the minio endpoint `http://minio.ic-shared-minio.svc.cluster.local:9000/` and bucket as `user1`
